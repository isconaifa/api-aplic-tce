...
func (repository *FiltroAcaoRepository) GetAllFiltroAcao(unidadeGestoraCodigo, ano string, acaoCodigo int) ([]models.FiltroAcao, error) {
	query := "SELECT DISTINCT\n    e.emp_data AS \"Emp_Data\",\n    e.emp_numero AS \"Emp_Numero\",\n " +
		...
		" v.mun_nome,\n " +
		"   v.ent_nome,\n    e.emp_numero,\n    e.emp_data"
	rows, err := repository.db.Query(query, unidadeGestoraCodigo, ano, acaoCodigo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var filtroAcoes []models.FiltroAcao
	for rows.Next() {
		var filtroAcao models.FiltroAcao
		if err := rows.Scan(
			&filtroAcao.Emp_Data,
			&filtroAcao.Emp_Numero,
			&filtroAcao.Credor,
			&filtroAcao.Valor_Empenhado,
			&filtroAcao.Valor_Liquidado,
			&filtroAcao.Valor_Pago,
			&filtroAcao.Valor_PagoRetencao,
			&filtroAcao.Anulado_Empenho,
			&filtroAcao.Qtde_Notas_Fiscais,
			&filtroAcao.Qtde_NFe,
			&filtroAcao.Contratos,
			&filtroAcao.Ent_codigo,
			&filtroAcao.Qtde_beneficiarios,
			&filtroAcao.Instrumento_contrato,
			&filtroAcao.Fonte_recurso_fonte,
			&filtroAcao.Tipo_concurso,
			&filtroAcao.Num_Concurso,
			&filtroAcao.Assistencia_social,
			&filtroAcao.Diarias,
			&filtroAcao.Relevante,
			&filtroAcao.Unidade_gestora,
			&filtroAcao.Municipio,
			&filtroAcao.Exercicio,
			&filtroAcao.Codigo_ug,
			&filtroAcao.Projeto_atividade,
			&filtroAcao.Dotacao,
			&filtroAcao.Num_obra,
			&filtroAcao.Tipo_serv_engenharia,
			&filtroAcao.Destinacao_recurso_grupo,
			&filtroAcao.Destinacao_recurso_iduso,
			&filtroAcao.Destinacao_recurso_codiog_iduso,
			&filtroAcao.Codigo_destinacao_recurso,
			&filtroAcao.Codigo_destinacao_recurso_especificacao,
			&filtroAcao.Destinacao_recurso_codigo_grupo,
			&filtroAcao.Fund_compra_direta_codigo,
			&filtroAcao.Fund_compra_direta_descricao,
			&filtroAcao.Destinacao_recurso,
			&filtroAcao.Orgao,
			&filtroAcao.UnidadeOrcamentaria,
			&filtroAcao.Unidade_orcamentaria_codigo,
			&filtroAcao.Funcao_codigo,
			&filtroAcao.Funcao_descricao,
			&filtroAcao.Subfuncao_codigo,
			&filtroAcao.Subfuncao_descricao,
			&filtroAcao.Programa_codigo,
			&filtroAcao.Num_Projeto_Atividade,
			&filtroAcao.Categoria_Economica,
			&filtroAcao.Natureza_Despesa,
			&filtroAcao.Modalidade_aplicacao_codigo,
			&filtroAcao.Elemento_despesa_codigo,
			&filtroAcao.Elemento_despesa_descricao,
			&filtroAcao.Subelemento_despesa_codigo,
			&filtroAcao.Subelemento_despesa,
			&filtroAcao.Descricao,
			&filtroAcao.Num_processo_licitatorio,
			&filtroAcao.Tipo_Contrato,
			&filtroAcao.Num_aditivo_contrato,
			&filtroAcao.Num_convenio,
			&filtroAcao.Num_aditivo_convenio,
			&filtroAcao.Compra_direta,
			&filtroAcao.Tipo,
			&filtroAcao.Mes_referencia,
			&filtroAcao.Identificacao_credor,
			&filtroAcao.Tipo_pesssoaCodigo,
		); err != nil {
			return nil, err
		}
		filtroAcoes = append(filtroAcoes, filtroAcao)
	}

	return filtroAcoes, nil
}
