...
func (repository *FiltroPeriodoEmpenhadoRepository) GetAllFiltroPeriodoEmpenhado(unidadeGestoraCodigo, ano, dataInicioStr, dataFimStr string) ([]models.FiltroPeriodoEmpenho, error) {
	query := "SELECT DISTINCT\n    e.emp_data AS empenho_data,\n    e.emp_numero AS empenho_numero,\n    c.cg_nome AS credor,\n    e.emp_valor - (\n        SELECT NVL(SUM(a.anul_valor), 0)\n        FROM aplic2008.anulacao_empenho a\n     " +
		...
		"AND e.ent_codigo = :1\n" +
		"AND e.exercicio = :2\n" +
		"AND e.emp_data BETWEEN to_date(:3, 'DD/MM/YYYY') \n" +
	...
	defer rows.Close()
	var filtroPeriodoEmpenhados []models.FiltroPeriodoEmpenho
	for rows.Next() {
		var f models.FiltroPeriodoEmpenho
		if err := rows.Scan(
			&f.Emp_Data,
			&f.Emp_Numero,
			&f.Credor,
		...
		filtroPeriodoEmpenhados = append(filtroPeriodoEmpenhados, f)
	}
	return filtroPeriodoEmpenhados, nil
}
