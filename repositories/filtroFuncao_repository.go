...
type FiltroFuncaoRepository struct {
	db *sql.DB
}

func NewFiltroFuncaoRepository(db *sql.DB) *FiltroFuncaoRepository {
	return &FiltroFuncaoRepository{db: db}
}

func (repository *FiltroFuncaoRepository) GetAllFiltroFuncao(unidadeGestoraCodigo, ano string, codigoFuncao int) ([]models.FiltroFuncao, error) {
	...
	defer rows.Close()
	var filtroFuncoes []models.FiltroFuncao

	for rows.Next() {
		var filtroFuncao models.FiltroFuncao
		if err := rows.Scan(
			&filtroFuncao.Emp_Data,
			&filtroFuncao.Emp_Numero,
			&filtroFuncao.Credor,
			&filtroFuncao.Valor_Empenhado,
			&filtroFuncao.Valor_Liquidado,
			&filtroFuncao.Valor_Pago,
			&filtroFuncao.Valor_PagoRetencao,
			&filtroFuncao.Anulado_Empenho,
			&filtroFuncao.Qtde_Notas_Fiscais,
			&filtroFuncao.Qtde_NFe,
			&filtroFuncao.Contratos,
			&filtroFuncao.Ent_codigo,
			&filtroFuncao.Qtde_beneficiarios,
			&filtroFuncao.Instrumento_contrato,
			&filtroFuncao.Fonte_recurso_fonte,
			&filtroFuncao.Tipo_concurso,
			&filtroFuncao.Num_Concurso,
			&filtroFuncao.Assistencia_social,
			&filtroFuncao.Diarias,
			&filtroFuncao.Relevante,
			&filtroFuncao.Unidade_gestora,
			&filtroFuncao.Municipio,
			&filtroFuncao.Exercicio,
			&filtroFuncao.Codigo_ug,
			&filtroFuncao.Projeto_atividade,
			&filtroFuncao.Dotacao,
			&filtroFuncao.Num_obra,
			&filtroFuncao.Tipo_serv_engenharia,
			&filtroFuncao.Destinacao_recurso_grupo,
			&filtroFuncao.Destinacao_recurso_iduso,
			&filtroFuncao.Destinacao_recurso_codiog_iduso,
			&filtroFuncao.Codigo_destinacao_recurso,
			&filtroFuncao.Codigo_destinacao_recurso_especificacao,
			&filtroFuncao.Destinacao_recurso_codigo_grupo,
			&filtroFuncao.Fund_compra_direta_codigo,
			&filtroFuncao.Fund_compra_direta_descricao,
			&filtroFuncao.Destinacao_recurso,
			&filtroFuncao.Orgao,
			&filtroFuncao.UnidadeOrcamentaria,
			&filtroFuncao.Unidade_orcamentaria_codigo,
			&filtroFuncao.Funcao_codigo,
			&filtroFuncao.Funcao_descricao,
			&filtroFuncao.Subfuncao_codigo,
			&filtroFuncao.Subfuncao_descricao,
			&filtroFuncao.Programa_codigo,
			&filtroFuncao.Num_Projeto_Atividade,
			&filtroFuncao.Categoria_Economica,
			&filtroFuncao.Natureza_Despesa,
			&filtroFuncao.Modalidade_aplicacao_codigo,
			&filtroFuncao.Elemento_despesa_codigo,
			&filtroFuncao.Elemento_despesa_descricao,
			&filtroFuncao.Subelemento_despesa_codigo,
			&filtroFuncao.Subelemento_despesa,
			&filtroFuncao.Descricao,
			&filtroFuncao.Num_processo_licitatorio,
			&filtroFuncao.Tipo_Contrato,
			&filtroFuncao.Num_aditivo_contrato,
			&filtroFuncao.Num_convenio,
			&filtroFuncao.Num_aditivo_convenio,
			&filtroFuncao.Compra_direta,
			&filtroFuncao.Tipo,
			&filtroFuncao.Mes_referencia,
			&filtroFuncao.Identificacao_credor,
			&filtroFuncao.Tipo_pesssoaCodigo,
		); err != nil {
			return nil, err
		}
		filtroFuncoes = append(filtroFuncoes, filtroFuncao)
	}
	return filtroFuncoes, nil
}
