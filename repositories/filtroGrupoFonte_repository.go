...
type FiltroGrupoFonteRepository struct {
	db *sql.DB
}

func NewFiltroGrupoFonteRepository(db *sql.DB) *FiltroGrupoFonteRepository {
	return &FiltroGrupoFonteRepository{db: db}
}

func (repository *FiltroGrupoFonteRepository) GetAllFiltroGrupoFonte(unidadeGestoraCodigo, ano string, codigoGrupoFonte int) ([]models.FiltroGrupoFonte, error) {
	query := "SELECT DISTINCT\n    e.emp_data AS Emp_Data,\n    e.emp_numero AS Emp_Numero,\n    c.cg_nome AS Credor,\n " +
		...
		" AND e.ent_codigo = :1\n " +
		"   AND e.exercicio = :2\n " +
		"   AND e.drgrp_codigo = :3\n" +
		"ORDER BY\n    v.mun_nome,\n    v.ent_nome,\n    e.emp_numero,\n    e.emp_data"

	rows, err := repository.db.Query(query, unidadeGestoraCodigo, ano, codigoGrupoFonte)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var grupofontes []models.FiltroGrupoFonte
	for rows.Next() {
		var grupoFonte models.FiltroGrupoFonte
		if err := rows.Scan(
			&grupoFonte.Emp_Data,
			&grupoFonte.Emp_Numero,
			&grupoFonte.Credor,
			&grupoFonte.Valor_Empenhado,
			&grupoFonte.Valor_Liquidado,
			&grupoFonte.Valor_Pago,
			&grupoFonte.Valor_PagoRetencao,
			&grupoFonte.Anulado_Empenho,
			&grupoFonte.Qtde_Notas_Fiscais,
			&grupoFonte.Qtde_NFe,
			&grupoFonte.Contratos,
			&grupoFonte.Ent_codigo,
			&grupoFonte.Qtde_beneficiarios,
			&grupoFonte.Instrumento_contrato,
			&grupoFonte.Fonte_recurso_fonte,
			&grupoFonte.Tipo_concurso,
			&grupoFonte.Num_Concurso,
			&grupoFonte.Assistencia_social,
			&grupoFonte.Diarias,
			&grupoFonte.Relevante,
			&grupoFonte.Unidade_gestora,
			&grupoFonte.Municipio,
			&grupoFonte.Exercicio,
			&grupoFonte.Codigo_ug,
			&grupoFonte.Projeto_atividade,
			&grupoFonte.Dotacao,
			&grupoFonte.Num_obra,
			&grupoFonte.Tipo_serv_engenharia,
			&grupoFonte.Destinacao_recurso_grupo,
			&grupoFonte.Destinacao_recurso_iduso,
			&grupoFonte.Destinacao_recurso_codiog_iduso,
			&grupoFonte.Codigo_destinacao_recurso,
			&grupoFonte.Codigo_destinacao_recurso_especificacao,
			&grupoFonte.Destinacao_recurso_codigo_grupo,
			&grupoFonte.Fund_compra_direta_codigo,
			&grupoFonte.Fund_compra_direta_descricao,
			&grupoFonte.Destinacao_recurso,
			&grupoFonte.Orgao,
			&grupoFonte.UnidadeOrcamentaria,
			&grupoFonte.Unidade_orcamentaria_codigo,
			&grupoFonte.Funcao_codigo,
			&grupoFonte.Funcao_descricao,
			&grupoFonte.Subfuncao_codigo,
			&grupoFonte.Subfuncao_descricao,
			&grupoFonte.Programa_codigo,
			&grupoFonte.Num_Projeto_Atividade,
			&grupoFonte.Categoria_Economica,
			&grupoFonte.Natureza_Despesa,
			&grupoFonte.Modalidade_aplicacao_codigo,
			&grupoFonte.Elemento_despesa_codigo,
			&grupoFonte.Elemento_despesa_descricao,
			&grupoFonte.Subelemento_despesa_codigo,
			&grupoFonte.Subelemento_despesa,
			&grupoFonte.Descricao,
			&grupoFonte.Num_processo_licitatorio,
			&grupoFonte.Tipo_Contrato,
			&grupoFonte.Num_aditivo_contrato,
			&grupoFonte.Num_convenio,
			&grupoFonte.Num_aditivo_convenio,
			&grupoFonte.Compra_direta,
			&grupoFonte.Tipo,
			&grupoFonte.Mes_referencia,
			&grupoFonte.Identificacao_credor,
			&grupoFonte.Tipo_pesssoaCodigo,
		); err != nil {
			return nil, err
		}
		grupofontes = append(grupofontes, grupoFonte)
	}
	return grupofontes, nil
}
