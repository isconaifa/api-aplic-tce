...
func (repository *FiltroDotacaoRepository) ObterFiltroDotacao(unidadeGestoraCodigo, ano, dotacao string) ([]models.FiltroDotacao, error) {
	partesDotacao := strings.Split(dotacao, ".")
	if len(partesDotacao) != 5 {
		return nil, fmt.Errorf("formato de dotacao inválida")
	}
	ctecCodigo := partesDotacao[0]
	ndespCodigo := partesDotacao[1]
	mdapCodigo := partesDotacao[2]
	eldeCodigo := partesDotacao[3]
	seldeCodigo := partesDotacao[4]
	...
	query := "SELECT DISTINCT\n    e.emp_data AS empenho_data,\n    e.emp_numero AS empenho_numero,\n " +
		...
		"AND e.exercicio = :2\n" +
		"AND e.ctec_codigo = :3\n" +
		"AND e.ndesp_codigo = :4\n" +
		"AND e.mdap_codigo = :5\n" +
		"AND e.elde_codigo = :6\n" +
		"AND e.selde_codigo = :7\n" +
		"ORDER BY v.mun_nome, v.ent_nome, e.emp_numero, e.emp_data"
	rows, err := repository.db.Query(query, unidadeGestoraCodigo, ano, ctecCodigo, ndespCodigo, mdapCodigo, eldeCodigo, seldeCodigo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var filtroDotacoes []models.FiltroDotacao
	for rows.Next() {
		var f models.FiltroDotacao
		if err := rows.Scan(
			&f.Emp_Data,
			&f.Emp_Numero,
			&f.Credor,
		...
		filtroDotacoes = append(filtroDotacoes, f)
	}
	return filtroDotacoes, nil
}
