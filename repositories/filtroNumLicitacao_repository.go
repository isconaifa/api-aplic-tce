...
func (repository *FiltroNumLicitacaoRepository) GetAllFiltroNumLicitacao(unidadeGestoraCodigo, ano, numLicitacao string) ([]models.FiltroNumLicitacao, error) {
	...
	rows, err := repository.db.Query(query, unidadeGestoraCodigo, ano, numLicitacao)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var filtroLicitacoes []models.FiltroNumLicitacao
	for rows.Next() {
		var f models.FiltroNumLicitacao
		if err := rows.Scan(
			&f.Emp_Data,
			&f.Emp_Numero,
			&f.Credor,
		...
		filtroLicitacoes = append(filtroLicitacoes, f)
	}
	return filtroLicitacoes, nil
}
