...
const FolhaPagamentParametrizada = "SELECT   p.fpgto_anoreferencia AS \"Ano de referência\",\n " +
	...
	"  FROM aplic2008.pessoal_folha_pagamento a  INNER JOIN aplic2008.pessoal pes\n     " +
	"  ON a.ent_codigo = pes.ent_codigo    AND pes.exercicio >= 2015\n     " +
	" AND a.pess_matricula = pes.pess_matricula\n  " +
	" WHERE a.ent_codigo = p.ent_codigo AND a.exercicio = p.exercicio\n   " +
	"AND a.fpgto_anoreferencia = p.fpgto_anoreferencia\n " +
	"AND a.fpgto_mesreferencia = p.fpgto_mesreferencia ) AS \"Qtde. funcionários\"\n" +
	"FROM aplic2008.folha_pagamento p INNER JOIN publico.meses m\n   " +
	" ON p.fpgto_mesreferencia = m.am_identificacao\n" +
	"WHERE p.ent_codigo = :unidadeGestoraCodigo\n  " +
	"AND p.exercicio = :ano\n " +
	"AND (:codigoOrgao IS NULL OR p.org_codigo = :codigoOrgao)\n " +
	"AND (:codigoUnidadeOrcamentaria IS NULL OR (:codigoOrgao IS NOT NULL AND p.unor_codigo = :codigoUnidadeOrcamentaria))\n " +
	"AND (:mesReferencia IS NULL OR p.fpgto_mesreferencia = :mesReferencia)\n" +
	"GROUP BY  p.ent_codigo, p.exercicio, p.fpgto_anoreferencia,  p.fpgto_mesreferencia,\n " +
	" m.am_descricao\n" +
