...
const FolhaPagamentoParametrizada = "SELECT p.fpgto_anoreferencia AS \"Ano de referência\",\n" +
	...
	"AND p.exercicio = :ano\n" +
	"and p.org_codigo = :codigoOrgao\n" +
	"and p.unor_codigo = :codigoUnidadeOrcamentaria\n" +
	"and p.fpgto_mesreferencia = :mesReferencia\n" +
	"GROUP BY p.ent_codigo, p.exercicio,\n" +
	"p.fpgto_anoreferencia, p.fpgto_mesreferencia,\n" +
