...
const NumConcurso = "select distinct e.ent_codigo, e.org_codigo as \"Órgão(código)\",                                                          \n       e.unor_codigo as \"Unidade Orçamentária(código)\",                                                   \n       e.emp_numero as \"N° do Empenho\",                                                                   \n   " +
	...
	"     e.cont_numaditivo as \"Nº do aditivo do contrato\",                                                  \n " +
	"      e.conv_numero as \"Nº do convênio\",                                                                 \n " +
	...
	"          and N.EMP_NUMERO = E.EMP_NUMERO) \"Qtde.Notas Fiscais\",                                         \n " +
	"       (select count(1)                                                                                  \n   " +
	"        FROM aplic2008.nota_fiscal n                                                                   \n    " +
	"      WHERE n.ent_codigo = e.ent_codigo                                                               \n     " +
	"       AND n.exercicio = e.exercicio                                                                 \n   " +
	...
	"           AND n.ntfsc_numeronfe is not null)  \"Qtde.NF-e\",                                              \n" +
	"        (select count(1)                                                                                  \n  " +
	...
	"          AND n.org_codigo = e.org_codigo                                                               \n " +
	"           AND n.unor_codigo = e.unor_codigo                                                             \n" +
	"            AND n.emp_numero = e.emp_numero)  \"Contrato(s)\",                                              \n" +
	...
	"       prj.prat_descricao as \"Projeto atividade\",                                                         \n" +
	"       e.ctec_codigo || '.' ||                                                                          \n  " +
	"     e.ndesp_codigo || '.' ||                                                                         \n " +
	"      e.mdap_codigo || '.' ||                                                                          \n " +
	"      e.elde_codigo || '.' ||                                                                          \n  " +
	"     e.selde_codigo as \"Dotação\",                                                                       \n  " +
	"     e.ent_codigo as \"Código da UG\",                                                                    \n " +
	"      e.exercicio as \"Exercício\",                                                                        \n" +
	"       v.mun_nome as \"Município\",                                                                         \n" +
	"       v.ent_nome as \"Unidade Gestora\",                                                                   \n " +
	"      '   ' as \"Relevante\",                                                                            \n   " +
	"    (select Count(1) from aplic2008.diaria dir                                                         \n  " +
	...
	"           and dir.emp_numero = e.emp_numero) as \"Diarias\"                                                ,FRC.FREC_DESCRICAO as \"Fonte de recurso - descrição\",                \n " +
	"      DECODE(e.emp_benefassistenciasocial, '1', 'Não existem beneficiários',         \n " +
	"      '2', 'Existem beneficiários sem cadastro informatizado',                       \n  " +
	"     '3', 'Existem beneficiários com cadastro informatizado')                       \n " +
	"      as \"Assistência Social\",                                                           \n" +
	"       e.conc_numero as \"N° do Concurso\",                                                 \n " +
	"      e.conc_tipo as \"Tipo do Concurso\",                                                 \n " +
	"      e.frec_codigo as \"Fonte de recurso - código\",                                      \n  " +
	"     decode(emp_instrumentocontrato,'1','Contrato','2',                           \n" +
	"       'Carta-Contrato','3','Nota de Empenho da Despesa','4',                     \n " +
	"      'Autorização de Compra','5','Ordem de Execução de Serviço','6',            \n   " +
	"    'Outros Instrumentos Hábeis') as \"Instrumento contrato\",                         \n " +
	"      t.trpps_descricao as \"Tipo da despesa(RPPS)\",                                      \n" +
	"       (select Count(1) from aplic2008.BENEF_ASSISTENCIA_SOCIAL_EMP bas                   \n " +
	"         where bas.ENT_CODIGO = E.ENT_CODIGO and bas.EXERCICIO = E.EXERCICIO             \n  " +
	...
	"           and a.emp_numero = e.emp_numero                                                ) as \"Valor Empenhado\", (select nvl(sum(a.anul_valor),0)              \n  " +
	"         from aplic2008.anulacao_empenho a                                              \n " +
	"         where a.ent_codigo = e.ent_codigo                                               \n " +
	"           and a.exercicio = e.exercicio                                                 \n " +
	"           and a.org_codigo = e.org_codigo                                               \n " +
	"           and a.unor_codigo = e.unor_codigo                                             \n  " +
	"          and a.emp_numero = e.emp_numero                                                ) as \"Anulado Empenho\"                                                  \n" +
	"  ,(select nvl(sum(al.anul_valor),0) from aplic2008.anulacao_liquidacao_empenho al        \n" +
	"         where al.ent_codigo = e.ent_codigo and al.exercicio = e.exercicio                \n " +
	"          and al.org_codigo = e.org_codigo and al.unor_codigo = e.unor_codigo            \n " +
	"          and al.emp_numero = e.emp_numero) as \"Anulado Liquidação\",                                                             \n " +
	"     (select nvl(sum(alp.anul_valor),0) from aplic2008.anulacao_pagamento_empenho alp,                 \n  " +
	"                                            aplic2008.pagamento_empenho_liquidacao pel                \n   " +
	"      where alp.ent_codigo = pel.ent_codigo and alp.exercicio = pel.exercicio                        \n  " +
	"         and alp.pgto_numero = pel.pgto_numero and pel.ent_codigo = e.ent_codigo                      \n " +
	"          and pel.org_codigo = e.org_codigo and pel.unor_codigo = e.unor_codigo                        \n " +
	"          and pel.emp_numero = e.emp_numero) as \"Anulado Pagamento\"                                                               \n" +
